...
# --- Watchdog to avoid "last 3s hang" and ensure auto-leave ---
end_guards = {}

async def _cancel_end_guard(chat_id: int):
    t = end_guards.get(chat_id)
    if t and not t.done():
        t.cancel()
        try:
            await t
            ...
            pass
    end_guards.pop(chat_id, None)

async def _start_end_guard(self, client, chat_id: int):
    # Start a timer that will change stream / leave when track should end,
    # in case on_stream_end doesn't fire reliably.
    await _cancel_end_guard(chat_id)
    check = db.get(chat_id) or []
    if not check:
        return
    cur = check[0]
    total = int(cur.get("seconds", 0) or 0)
    played = int(cur.get("played", 0) or 0)
    # keep a small margin to avoid tail blocking
    remaining = max(1, total - played - 1)
    async def _guard():
        try:
            ...
            if not q:
                ...
                return
            # cancel the current guard before forcing next
            await _cancel_end_guard(chat_id)
            try:
            ...
            pass
    end_guards[chat_id] = asyncio.create_task(_guard())
# --- end watchdog block ---
            ...
            if is_video:
                if AudioVideoPiped != str:
                    return AudioVideoPiped(
                        ...
                        additional_ffmpeg_parameters=self._to_args(additional_params),
                    )
                else:
...
stype = StreamType().local_stream  # treat as file/finite by default
joined = False
for method_name in join_methods:
    if hasattr(assistant, method_name):
        try:
            method = getattr(assistant, method_name)
            if method_name in ("join_group_call", "join_call"):
                await method(chat_id, stream, stream_type=stype)
            else:
...
if not joined:
    raise AssistantErr("Could not join call - no working method found")
        ...
        await _start_end_guard(self, assistant, chat_id)
        if await is_autoend():
            counter[chat_id] = {}
            try:
    ...
    async def seek_stream(self, chat_id, file_path, to_seek, duration, mode):
        assistant = await group_assistant(self, chat_id)
        length = max(0, int(duration) - int(to_seek))
        additional_params = f"-ss {to_seek} -t {length} -nostats -loglevel error -fflags nobuffer -flags low_delay -flush_packets 1 -max_interleave_delta 0"
    ...
    async def speedup_stream(self, chat_id: int, file_path, speed, playing):
        ...
        additional_params = f"-ss {played} -t {max(0, int(dur) - int(played))} -nostats -loglevel error -fflags nobuffer -flags low_delay -flush_packets 1 -max_interleave_delta 0"
    ...
    async def change_stream(self, client, chat_id):
        ...
        try:
            ...
            if "live_" in queued:
                ...
                await _start_end_guard(self, client, chat_id)
                                ...
                                try:
                                    if hasattr(update, 'chat_id'):
                                        ...
                                        if not check or len(check) == 0:
                                            ...
                                            # Hủy guard cũ trước khi đổi bài để tránh đúp
                                            await _cancel_end_guard(chat_id)
